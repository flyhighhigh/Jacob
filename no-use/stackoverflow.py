...
class Music(commands.Cog):
    ...
    async def queue_info(self, ctx):
        ...
        if player.queue.empty():
            embed = discord.Embed(title="", description="**無待播曲目**", color=discord.Color.green())
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(title=f'播放清單 | {ctx.guild.name}', description=fmt, color=discord.Color.green())
